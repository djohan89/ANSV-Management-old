...
public class ProjectDao extends BaseDao {
	...
	public List<DashboardProjectsDto> getAllByWeek(int week) {
		...
		String sql = "SELECT project.id, project.week, projects_types.name AS type, priorities.name AS priority, projects_status.name AS status, "
				...
				+ "WHERE users.id = pic_id AND (role.name = 'ROLE_AM' OR role.name = 'ROLE_PM')) AS pic_role,project.pham_vi_cung_cap,"
				+ "project.tong_gia_tri_thuc_te,project.DAC,project.PAC,project.FAC,project.so_tien_tam_ung,project.ke_hoach_tam_ung,"
	...
	public List<ProjectDetailDto> getAllProjectByCustomerAndWeek(int week, int customer_id){
		...
		list = _jdbcTemplate.query(sql, new ProjectDetailDtoMapper(),week,customer_id);
		return list;
