...
public class ProjectTypesDao extends BaseDao {
	...
	public List<ProjectTypes> getAll() {
		String sql = "SELECT id, name, display, COUNT(*) AS number FROM projects_types GROUP BY name";
	...
	public List<ProjectTypes> getMenu(int week, int year) {
		String sql = "SELECT projects_types.id, projects_types.name, projects_types.display, COUNT(*) AS number FROM projects_types "
