									...
									<c:forEach var="item" items="${customer }" varStatus="index">
										<li class="nav-item item">
											<c:if test="${index.first }">
												<a class="dropdown-item nav-link active" href="#du_an_${index.count}" type="button">
													${item.name }
	                							</a>
											</c:if>
											<c:if test="${not index.first }">
												<a class="dropdown-item nav-link" href="#du_an_${index.count}" type="button">
													${item.name }
	                							</a>
											</c:if> 
										</li>
								...
								<c:if test="${not index.first }">
									<div id="du_an_${index.count }" class="tab-pane fade"><br>
										...
										<div>
											...
											<p>
												<b>Phạm vi cung cấp:</b> <span style="white-space: pre-wrap;">${item.pham_vi_cung_cap }</span>
											</p>
											<p>
												<b>Tổng giá trị:</b>
								...
								<div class="d-flex">
									<div style="width: 70%;">
										...
										<p class="text-justify">
											${item.tinh_trang_va_ke_hoach_chi_tiet }</p>
									</div>
									<div class="pl-5">
										<p>
